# Yemen Commerce AI Execution Engine and Typed Tool Dispatcher

**Status:** AI-1/AI-2 are deployed in `ai-run`; AI-3 proposal/execution, AI-4 creator controls, AI-5 managed knowledge foundations, and AI-6 workflow/MCP foundations are deployed as separate gated functions and migrations. Provider calls, background workflows, managed knowledge, and external-agent access remain disabled by default until their explicit controls and isolated test gates pass.
...
The AI-1/AI-2 read-only registry intentionally does not expose mutation commands, catalog writes, checkout, payment, COD collection, courier dispatch, promotion application, evidence lookup, private Storage reads, arbitrary SQL/RPC, web fetches, or external-agent calls. AI-3 mutation proposals are exposed only through separate approval-bound functions documented in `docs/AI3_AI6_IMPLEMENTATION.md`. AI-2 drafting returns a validated in-memory envelope for human review; it never saves, publishes, applies, or approves a draft. B2B and order tools remain bounded projections, and raw buyer identifiers or contact details are excluded. A recursive redactor is an additional defense, not a replacement for database projection design.
...
The active `ai-run` function is deployed with JWT verification enabled; AI-3 proposal and execution, AI-6 MCP, and AI-6 worker functions are separately deployed with their documented authentication modes. The current versions are `ai-run` v6, `ai-propose-action` v4, `ai-execute-action` v3, `ai-mcp` v2, and `ai-workflow-worker` v1. Anonymous probes must return HTTP 401 or the documented internal-worker denial before any protected operation. A maintainer should upload `supabase/functions/ai-run/index.ts`, `contract.ts`, and `deno.json` as one version, then verify that the function is `ACTIVE` and `verify_jwt = true`. Provider secrets should be configured separately through the Supabase project’s secret-management path, never in the repository or deployment JSON. The function can be rolled back by deploying the last reviewed source version; database migrations remain ordered and are not rewritten.

The AI database prerequisites are migrations `20260826_0044_ai1_effective_policy.sql` through `20260826_0058_ai5_terminology.sql`. Migrations 0046–0055 add the reviewable merchant-action registry, creator controls, managed knowledge, evaluations, and server-retained owner-scoped action payloads. Migrations 0056–0057 add the consent-bound per-user/per-client daily external-agent quota and explicit deny RLS policy. Security Advisor is clean. Performance Advisor reports only informational unused-index notices and no errors or warnings for the AI roadmap.
...
The existing `supabase/tests/run_creator_authorization.sh` now includes anonymous denial for the AI-3 through AI-6 public RPCs, including the action-definition/payload gates and quota path. The connected anonymous run passed 113 checks. Authenticated AI-1 through AI-6 tests remain intentionally gated behind an isolated project or branch and short-lived customer, merchant, and creator tokens. They must not create users or mutate fixtures in the connected shared project. Future authenticated vectors must prove that a customer cannot request merchant tools, a merchant cannot read another shop, and a developer path requires creator authorization.

## Explicit non-goals

AI-2 itself does not add automated payment verification, platform custody, order mutation, draft publication, or direct table writes. AI-3 through AI-6 add separate gated foundations for reviewable actions, creator governance, managed knowledge, resumable workflows, and read-only external-agent access; none bypasses the existing RPC/RLS/audit boundaries. Full provider, scheduler, OAuth broker, and isolated authenticated release gates remain required before enabling those controls. The system continues to treat Jaib as a manual QR/POS/reference payment flow until official API, callback, settlement, refund, and compliance documentation is approved.
