...
|---|---:|---:|---:|---:|---:|
| In-product AI assistant with live commerce context | **1/5** — no production AI runtime yet; strong RPC/RLS foundation | **4/5** — Zid AI and MCP connection are documented | **5/5** — Sidekick is embedded in admin and can guide and act | **4/5** — conversational and agentic commerce capabilities documented | **4/5** — Companion and brand-agent direction documented |
...
| Agentic admin mutations | **1/5** now; target **4/5** after approval gateway | **4/5** — MCP app claims 145+ tools and configurable tool access | **5/5** — Sidekick can complete admin tasks, with review and permission respect | **4/5** — Storefront MCP and agentic standards direction | **3/5** — Companion/brand-agent direction, details vary |
| Customer conversational shopping | **1/5** now; target **4/5** with scoped catalog/order tools | **2–3/5** in reviewed sources; merchant AI is clearer than customer agent capability | **3/5** in reviewed sources; Sidekick is primarily admin-facing | **4/5** — conversational shopping guidance, recommendations, comparisons, real-time price/inventory | **4/5** — AI shopping and in-conversation buying positioning |
...
Yemen Commerce is **not yet close in visible AI feature breadth** because the current system has no production model gateway, customer copilot, merchant copilot, AI-generated content, semantic search, background AI runs, or agent protocol surface. On that dimension, Shopify and Zid are ahead in practical merchant-facing AI, while Adobe and BigCommerce are ahead in enterprise and agentic-commerce positioning.
...
|---|---|---|
| AI-0 | Add AI run/tool/approval/audit schema, server gateway skeleton, model/provider config, budgets, redaction, and Arabic-safe errors. | No mutations. |
| AI-1 | Customer read-only assistant: product discovery, policy answers, own-order status, delivery/COD explanation, cited retrieval. | Customer scope only. |
| AI-2 | Merchant read-only and drafting copilot: workbench, inventory, rollups, B2B, product copy, SEO, image instructions, quote drafts. | Merchant scope; drafts only. |
...
The connected Supabase project accepted all three migrations. The Security Advisor recheck is clean (`lints: []`). The Performance Advisor reports no errors or warnings or new unindexed foreign-key findings; its remaining notices are informational unused-index records, including AI-0 indexes that have not yet been exercised by live traffic. The bounded structural check confirms all four tables have RLS enabled and one explicit policy each, and confirms the public lifecycle wrappers exist. The anonymous authorization suite now reports `RESULT passed=95 skipped=5`; the five skips are the existing authenticated synthetic-user cases, not anonymous AI-0 access.

This is a **database control plane only**. No LLM provider, model call, retrieval source, background agent, external MCP server, payment action, evidence retrieval path, or direct table-write path is enabled by AI-0. The next increment is the server-side AI gateway, which must use these run/tool/approval/policy records and existing commerce RPCs rather than bypassing them.
