...
| AI-1b | Add customer catalog/order projections, citations, typed Flutter invocation/status contracts, and isolated authenticated tests. | Customer scope only. |
...
The connected Supabase project accepted all four AI migrations. The Security Advisor recheck is clean (`lints: []`). The Performance Advisor reports no errors or warnings or new unindexed foreign-key findings; 174 remaining notices are informational unused-index records, including AI-0 indexes that have not yet been exercised by live traffic. The bounded structural check confirms the effective-policy resolver exists in both `private` and `public`, with a `SECURITY DEFINER` implementation and a narrow public wrapper. The anonymous authorization suite now includes the AI-1 resolver denial case; the authenticated AI-1 cases remain gated behind isolated short-lived tokens and fixtures.

AI-1 is now implemented as `supabase/functions/ai-run` and deployed with JWT verification enabled. Migration `20260826_0044_ai1_effective_policy.sql` supplies the authenticated effective-policy resolver. The gateway uses `ai_start_run`, `ai_get_effective_policy`, `ai_propose_tool_call`, `ai_transition_tool_call`, and `ai_finish_run`; it accepts only fixed read-only tool names, passes the verified bearer token to existing read-only RPCs, redacts sensitive keys, and returns Arabic-safe errors. The current version deliberately returns `AI_PROVIDER_NOT_CONFIGURED` until an approved provider URL/key/model is configured as server-side Function secrets and the corresponding policy is enabled. No payment action, evidence retrieval path, direct table write, background agent, external MCP server, or mutation tool is enabled. The next increments are safe catalog/order projections, typed Flutter client contracts, isolated authenticated tests, and then reviewable drafting/mutation workflows.
