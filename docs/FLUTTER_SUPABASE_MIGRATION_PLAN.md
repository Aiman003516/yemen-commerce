...
## 12. Extended production and creator-console roadmap

The full remaining production scope and the separate third Flutter application for creator-led people and platform management are documented in [PRODUCTION_COMPLETION_AND_CREATOR_APP_PLAN.md](./PRODUCTION_COMPLETION_AND_CREATOR_APP_PLAN.md). That roadmap adds creator-only roles and capabilities, people management, market and policy governance, delegated operator controls, synthetic-user RLS coverage, Realtime and notifications, observability, platform release gates, legacy runtime removal, and Ibb pilot validation.
