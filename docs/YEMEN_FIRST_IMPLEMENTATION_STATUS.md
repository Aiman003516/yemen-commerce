...
The Supabase Security Advisor returned no lints after migrations 0027 and 0028. Performance Advisor no longer reports the 63 unindexed foreign-key findings or the eight multiple-permissive-policy findings; it now reports 128 informational unused-index findings, including the newly created FK and command-key indexes that have not yet accumulated production query usage. These are not grounds for immediate removal because the indexes were added for referential checks, RLS joins, and future operational queries; review them after representative traffic and query-plan measurements. The anonymous authorization runner covers the four analytics/export RPCs and the three-argument idempotent promotion RPC, as well as creator, provider, address, variant, order-case, courier, review, promotion, and notification RPCs, plus protected creator tables. Authenticated synthetic customer, merchant, review-agent, support-agent, and creator checks remain skipped until isolated access tokens or an isolated test project are supplied. Customer/Creator Console analysis/tests and production Web builds remain the required validation gates; Android/iOS device and archive validation are still platform-toolchain gates.
