...
The next increments should add replay visibility/sync diagnostics, courier provisioning and richer dispatch filtering, review moderation list UI, storefront/theme rendering, full multi-location stock write/transfer/count workflows, POS export and reconciliation reports, B2B price-list item editing and negotiated checkout pricing, provider event ingestion through a server-side adapter, approved wallet adapters, and Android/iOS release validation where platform toolchains are available. Provider adapters must remain independently gated by credentials, terms, webhook verification, consent, settlement behavior, and compliance review; the current provider pages are mock/demo surfaces only.
