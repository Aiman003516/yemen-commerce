...
| Merchant operations | Implemented subset | The merchant workspace reads `merchant.mine` and writes merchant-owned shop, manual payment-method, collection-fulfilment, and paid-order readiness settings through protected routes. It contains no client-side authorization or payment decision logic; a shop remains pending until an administrator separately approves it, and fulfilment cannot progress before the server confirms payment. |
| Split checkout | Implemented subset | The grouped cart receives active fulfilment and manual payment choices per shop/merchant, requires one of each for every group, presents a split-order confirmation, and submits them to `cart.checkout`. The server remains responsible for availability, ownership, payment-method, and independent-order validation. |
