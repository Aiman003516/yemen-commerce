...
class MarketplaceApiClient {
  ...
  SupabaseMarketplaceClient? get _supabase => SupabaseConfig.isConfigured ? SupabaseMarketplaceClient() : null;
  ...
  Future<MarketConfig> activeMarket() async {
    ...
    if (supabase != null) {
      ...
      return MarketConfig(id: market.id, governorate: market.governorate, city: market.city, currency: market.currency, isPilot: market.isPilot);
    }
    final response = await _client.get(_uri('/api/trpc/market.active'));
    if (response.statusCode < 200 || response.statusCode >= 300) throw ApiException('تعذر تحميل إعدادات السوق.');
    final decoded = jsonDecode(response.body) as Map<String, dynamic>;
    final result = decoded['result'] as Map<String, dynamic>;
    final data = result['data'] as Map<String, dynamic>;
    return MarketConfig.fromJson((data['json'] ?? data) as Map<String, dynamic>);
  }
  ...
  Future<List<MarketplaceProduct>> products({String? query}) async {
    ...
    if (supabase != null) {
      ...
      final products = await supabase.products(query: query, marketId: market.id);
      return products.map((product) => MarketplaceProduct(id: product.id, name: product.name, priceMinor: product.priceMinor, currency: product.currency, stockQuantity: product.stockQuantity, shopName: product.shopName, shopSlug: product.shopSlug)).toList(growable: false);
    }
    ...
    if (response.statusCode < 200 || response.statusCode >= 300) throw ApiException('تعذر تحميل كتالوج المنتجات.');
    final decoded = jsonDecode(response.body) as Map<String, dynamic>;
    final result = decoded['result'] as Map<String, dynamic>;
    final data = result['data'] as Map<String, dynamic>;
    final json = (data['json'] ?? data) as List<dynamic>;
    return json.map((item) => MarketplaceProduct.fromCatalogJson(Map<String, dynamic>.from(item as Map))).toList();
  }
  ...
  Future<SessionUser?> currentUser() async {
    ...
    if (supabase != null) {
      ...
      return SessionUser(id: user.id, name: profile?.displayName ?? user.email, role: roles.contains('admin') ? 'admin' : roles.contains('merchant') ? 'merchant' : 'customer');
    }
    ...
    return json == null ? null : SessionUser.fromJson(Map<String, dynamic>.from(json as Map));
  }
  ...
  Future<void> submitMerchantApplication({required String phone, required String ownerName}) async {
    ...
    if (supabase != null) {
      final market = await supabase.activeMarket();
      if (market == null) throw ApiException('لا يوجد سوق نشط حالياً.');
      await supabase.submitMerchantApplication(phone: phone, ownerName: ownerName, marketId: market.id);
      return;
    }
    final response = await _client.post(_uri('/api/trpc/merchant.submitApplication'), headers: {'Content-Type': 'application/json'}, body: jsonEncode({'json': {'phone': phone, 'ownerName': ownerName}}));
    if (response.statusCode < 200 || response.statusCode >= 300) throw ApiException('تعذر إرسال طلب التاجر. تحقق من تسجيل الدخول والبيانات.');
  }
  ...
  Future<MerchantWorkspace> merchantWorkspace() async {
    final supabase = _supabase;
    if (supabase != null) return MerchantWorkspace.fromJson(await supabase.merchantWorkspace());
    final response = await _client.get(_uri('/api/trpc/merchant.mine'));
    if (response.statusCode < 200 || response.statusCode >= 300) throw ApiException('تعذر تحميل مساحة التاجر.');
    final decoded = jsonDecode(response.body) as Map<String, dynamic>;
    final data = (decoded['result'] as Map<String, dynamic>)['data'] as Map<String, dynamic>;
    return MerchantWorkspace.fromJson(Map<String, dynamic>.from((data['json'] ?? data) as Map));
  }
  ...
  Future<void> createShop({required String name, required String slug, required String areaLabel}) async {
    ...
    if (supabase != null) {
      final market = await supabase.activeMarket();
      if (market == null) throw ApiException('لا يوجد سوق نشط حالياً.');
      await supabase.createShop(name: name, slug: slug, areaLabel: areaLabel, marketId: market.id);
      return;
    }
  ...
  Future<void> saveProduct({String? id, required String shopId, String? categoryId, required String name, required String description, required int priceMinor, required int stockQuantity, required String status}) async {
    final supabase = _supabase;
    if (supabase != null) {
      await supabase.saveProduct(id: id, shopId: shopId, categoryId: categoryId, name: name, description: description, priceMinor: priceMinor, stockQuantity: stockQuantity, status: status);
      return;
    }
    final response = await _client.post(_uri('/api/trpc/merchant.saveProduct'), headers: {'Content-Type': 'application/json'}, body: jsonEncode({'json': {'id': id, 'shopId': shopId, 'categoryId': categoryId, 'name': name, 'description': description, 'priceMinor': priceMinor, 'stockQuantity': stockQuantity, 'status': status}}));
    if (response.statusCode < 200 || response.statusCode >= 300) throw ApiException('تعذر حفظ المنتج.');
  }

  Future<void> saveMerchantPaymentMethod({String? id, required String name, required String accountHolderName, required String receivingIdentifier, required String instructions, required String proofRequirement}) async {
    final supabase = _supabase;
    if (supabase != null) {
      await supabase.saveMerchantPaymentMethod(id: id, name: name, accountHolderName: accountHolderName, receivingIdentifier: receivingIdentifier, instructions: instructions, proofRequirement: proofRequirement);
      return;
    }
  ...
  Future<void> setMerchantFulfilment({required String shopId, required String method, required String instructions, required bool isActive}) async {
    final supabase = _supabase;
    if (supabase != null) {
      await supabase.setMerchantFulfilment(shopId: shopId, method: method, instructions: instructions, isActive: isActive);
      return;
    }
  ...
  Future<void> reviewPaymentClaim({required String merchantOrderId, required bool approve, String? reason}) async {
    final supabase = _supabase;
    if (supabase != null) {
      await supabase.reviewPaymentClaim(merchantOrderId: merchantOrderId, approve: approve, reason: reason);
      return;
    }
    final response = await _client.post(_uri('/api/trpc/payment.reviewClaim'), headers: {'Content-Type': 'application/json'}, body: jsonEncode({'json': {'merchantOrderId': merchantOrderId, 'decision': approve ? 'paid' : 'rejected', 'reason': reason}}));
    if (response.statusCode < 200 || response.statusCode >= 300) throw ApiException('تعذر حفظ قرار مراجعة الدفع.');
  }

  Future<void> updateMerchantFulfilment({required String merchantOrderId, required String fulfilmentStatus}) async {
    final supabase = _supabase;
    if (supabase != null) {
      await supabase.updateMerchantFulfilment(merchantOrderId: merchantOrderId, fulfilmentStatus: fulfilmentStatus);
      return;
    }
  ...
  Future<IdentityVerificationSummary> identityMine() async {
    final supabase = _supabase;
    if (supabase != null) return IdentityVerificationSummary.fromJson(await supabase.identityMine());
    ...
    final decoded = jsonDecode(response.body) as Map<String, dynamic>;
    final data = (decoded['result'] as Map<String, dynamic>)['data'] as Map<String, dynamic>;
    return IdentityVerificationSummary.fromJson(Map<String, dynamic>.from((data['json'] ?? data) as Map));
  }
  ...
  Future<void> submitIdentityEvidence({required String passportBase64, required String passportName, required String passportMimeType, required String selfieBase64, required String selfieName, required String selfieMimeType}) async {
    final supabase = _supabase;
    if (supabase != null) {
      await supabase.submitIdentityEvidenceFromBase64(passportBase64: passportBase64, passportName: passportName, passportMimeType: passportMimeType, selfieBase64: selfieBase64, selfieName: selfieName, selfieMimeType: selfieMimeType);
      return;
    }
  ...
  Future<List<AdminIdentityCase>> adminIdentityQueue() async {
    ...
    if (supabase != null) {
      ...
      return rows.map((row) => AdminIdentityCase.fromJson(row)).toList(growable: false);
    }
    ...
    final data = (decoded['result'] as Map<String, dynamic>)['data'] as Map<String, dynamic>;
    final json = (data['json'] ?? data) as List<dynamic>;
    return json.map((item) => AdminIdentityCase.fromJson(Map<String, dynamic>.from(item as Map))).toList();
  }

  Future<List<AdminIdentityEvidence>> adminIdentityEvidence(String identityCaseId) async {
    final supabase = _supabase;
    if (supabase != null) {
      final rows = await supabase.adminIdentityEvidence(identityCaseId);
      return rows.map((row) => AdminIdentityEvidence.fromJson(row)).toList(growable: false);
    }
    ...
    final data = (decoded['result'] as Map<String, dynamic>)['data'] as Map<String, dynamic>;
    final json = (data['json'] ?? data) as List<dynamic>;
    return json.map((item) => AdminIdentityEvidence.fromJson(Map<String, dynamic>.from(item as Map))).toList();
  }

  Future<void> reviewIdentityCase({required String identityCaseId, required bool approve, required String note}) async {
    final supabase = _supabase;
    if (supabase != null) {
      await supabase.reviewIdentityCase(identityCaseId: identityCaseId, approve: approve, note: note);
      return;
    }
  ...
  Future<List<MerchantOrderSummary>> myOrders() async {
    ...
    if (supabase != null) {
      ...
      return rows.map((row) => MerchantOrderSummary.fromJson(row)).toList(growable: false);
    }
    ...
    final data = result['data'] as Map<String, dynamic>;
    final json = (data['json'] ?? data) as List<dynamic>;
    return json.map((item) => MerchantOrderSummary.fromJson(Map<String, dynamic>.from(item as Map))).toList();
  }
  ...
  Future<List<CartGroup>> cartGroups() async {
    ...
    if (supabase != null) {
      ...
      return groups.map((group) => CartGroup.fromJson(group)).toList(growable: false);
    }
    ...
    final data = (decoded['result'] as Map<String, dynamic>)['data'] as Map<String, dynamic>;
    final json = (data['json'] ?? data) as Map<String, dynamic>;
    return (json['groups'] as List<dynamic>).map((group) => CartGroup.fromJson(Map<String, dynamic>.from(group as Map))).toList();
  }
  ...
  Future<void> checkoutCart({required List<Map<String, dynamic>> fulfilmentByShop, required List<Map<String, dynamic>> paymentMethodByMerchant}) async {
    ...
    if (supabase != null) {
      final market = await supabase.activeMarket();
      if (market == null) throw ApiException('لا يوجد سوق نشط حالياً.');
      await supabase.checkoutCreateOrders(marketId: market.id, fulfilmentByShop: fulfilmentByShop, paymentByMerchant: paymentMethodByMerchant);
      return;
    }
  ...
  Future<void> submitPaymentReference({required String merchantOrderId, required String reference}) async {
    final supabase = _supabase;
    if (supabase != null) {
      await supabase.submitPaymentClaim(merchantOrderId: merchantOrderId, transactionReference: reference);
      return;
    }
