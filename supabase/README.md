...
20260826_0045_ai2_merchant_projection.sql
20260826_0059_erp_foundation.sql
20260826_0060_erp_fk_indexes.sql
20260826_0061_erp_merchant_views.sql
20260826_0062_erp_accounting_rpc.sql
20260826_0063_erp_organization_scope_fix.sql
20260826_0064_erp_operational_projections.sql
20260826_0065_erp_operational_fk_indexes.sql
20260826_0066_erp_journal_scope_hardening.sql
20260826_0067_erp_authoring_reason_contract.sql
20260827_0068_composable_erp_core.sql
20260827_0069_composable_erp_fk_indexes.sql
20260827_0070_composable_visibility_hardening.sql
20260827_0071_universal_journal_creator_only.sql
20260827_0072_channels_logistics_returns.sql
20260827_0073_channels_logistics_fk_indexes.sql
20260827_0074_public_channel_visibility.sql
20260827_0075_channels_logistics_idempotency.sql
  ```
...
Migrations 0009 through 0043 extend the pilot with market service areas, pickup points, private customer address ownership, merchant delivery zones, product variants, customer order cases for cancellation/return/dispute review, courier assignment and handoff operations, durable recipient-scoped notification events, completed-order product reviews, merchant promotion configuration, storefront/theme settings, inventory locations, aggregate merchant analytics, delivery-address and pickup snapshots at checkout, location-level inventory reservations, append-only cash-on-delivery reconciliation, server-side reservation release/finalization commands, support tickets, internal risk signals, explainable merchant quality metrics, provider catalog/configuration metadata, webhook-event storage foundations, staged market feature rollouts, Creator service-area/pickup governance, review moderation, loyalty ledgers, B2B business profiles/wholesale requests, local POS sessions/sales, retry-safe checkout command keys, server-side promotion application, courier dispatch events, B2B negotiated price lists and review approvals, immutable quote versions, negotiated checkout application, daily merchant rollups, private product-image variant metadata and completion, provider adapter-operation gates, POS close/reconciliation, merchant B2B/POS aggregates, bounded privacy-safe export RPCs, audit events, shop/date indexes, remediation for foreign-key findings and multiple-permissive-policy findings, explicit export pagination guards, server-side promotion command idempotency with an explicit client-deny policy for its internal key table, audited location-level inventory adjustments, atomic transfers and stock counts, barcode-aware product saves, bounded atomic catalog imports, product-stock consistency guards, targeted inventory foreign-key indexes, merchant order-workbench projections, COD reconciliation batches with append-only batch-linked collection records, bounded operational filters, and COD foreign-key indexes. These additions remain optional and fail safely when no service area, courier, notification, storefront, external provider adapter, or AI runtime is configured. Migrations 0041–0043 add the AI-0 control plane: append-only core fields for authenticated AI runs, typed tool-call lifecycle records, resumable approval records, versioned creator-published policies, conservative system policy defaults, scope-derived lifecycle RPCs, creator-only policy publication, bounded run/tool/approval projections, audit events, immutable-field triggers, RLS, explicit grants, and the required policy-author and lifecycle indexes. Migration 0044 adds the authenticated, read-only effective-policy resolver used by the AI gateway. Migration 0045 adds the bounded merchant-owned `merchant_ai_catalog` projection for product-copy context without customer identity, payment evidence, or private storage keys. AI-1 adds `supabase/functions/ai-run`, deployed with JWT verification enabled. AI-2 extends it with merchant-only read/draft modes, B2B/COD/POS/quality analytics tools, strict draft envelopes, and a typed Flutter client plus Arabic-first merchant review card. Drafts remain in-memory responses and are never saved, published, or applied. The gateway starts and finishes AI-0 runs, resolves policy per run and tool, dispatches only fixed read-only projections through the caller's bearer token, recursively redacts identity/contact/evidence fields, caps tool rounds, and returns Arabic-safe unavailable responses while provider secrets or policy permission are absent. It does not call payment providers, retrieve evidence, write operational tables, or accept arbitrary SQL/RPC/URLs. See `docs/AI1_EXECUTION_ENGINE.md` for the allowlist, secret names, deployment, and isolated authenticated test gates. The merchant UI continues to support bounded multi-line inventory transfers, camera barcode capture through `mobile_scanner`, B2B price-list line editing, versioned quote creation, daily rollup refresh, and optional private catalog-image optimization. Android declares the camera permission and iOS declares `NSCameraUsageDescription`. Web camera use depends on browser permission and supported camera/detection APIs. Migrations 0046–0058 complete the AI-3 through AI-6 foundations: creator-controlled merchant action definitions; separate proposal, approval, and execution paths using fixed existing RPCs; owner-scoped server-retained action payloads with redacted client projections; versioned Creator AI settings; managed scope-aware knowledge sources/chunks with provenance and full-text retrieval; Arabic terminology memory with canonical terms and aliases; evaluation suites/cases/runs/results and creator summaries; resumable workflow leases; external-client registration, exact redirect consent and revocation; and an atomic 120-request-per-UTC-day quota per consenting user/client pair. Migrations 0059–0071 add the 32-feature ERP foundation and the first operational/composable vertical slices: entity and book accounting, immutable journal authoring/posting, dimensions, revenue schedules, consolidation/allocation/fund/project structures, procure-to-pay and reconciliation records, CRM/CLM/subscriptions/field-service structures, provider adapters, event outbox/webhook/workflow tables, graph and analytics rollups, merchant-safe feature catalog/dashboard RPCs, typed Flutter adapters, a logical bounded-context/module registry, versioned event envelopes, inbox deduplication, projection checkpoints, a universal-journal projection for posted journal lines, and reviewed metadata-only extension manifests. No external event sender, Kafka/CDC/ClickHouse pipeline, WASM runtime, or provider activation is enabled. Universal-journal projection rows are Creator-only at both the RPC and direct-table policy layers. The Creator Console exposes the complete ERP capability map, bounded organization dashboard, and reason-required authoring panel for organizations, legal entities, books, accounts, and two-line draft journals with separate posting confirmation; the merchant app exposes its own ERP capability map and safe summary counts. Migration 0060 adds the reported foreign-key indexes; migrations 0061–0062 add merchant views and accounting authoring RPCs; migration 0063 closes organization uniqueness-conflict updates across merchant scopes; migration 0064 adds scoped operational projection tables for intercompany netting, AR invoices/collections, dunning drafts, customer 360, vendor quote/negotiation drafts, CPQ quotes, and field assignments; migration 0065 adds their covering foreign-key indexes; migration 0066 hardens journal-line organization validation; migration 0067 requires audit reasons for Creator ERP authoring mutations. The Creator Console now provides reason-required settings/action governance, source/chunk ingestion, and evaluation summaries, while the merchant app provides a redacted approval inbox with separate approve/reject and execute steps. Provider calls, background workers, managed knowledge retrieval, and MCP access remain disabled by default. `ai-run` v6, `ai-propose-action` v4, `ai-execute-action` v3, and `ai-mcp` v2 are active with JWT verification; `ai-workflow-worker` v1 remains unavailable until server-only worker secrets are intentionally configured.

The final live Security Advisor result after migrations through 0075 is `lints: []`. Performance Advisor remediation through migration 0069 and 0073 removed the new composable ERP and 0072 operational foreign-key findings; the post-0075 scan contains 364 informational `unused_index` observations only, with no errors or warnings and no remaining new operational unindexed-FK findings. These observations do not change authorization semantics. Expected access-path indexes are retained for production traffic. Bounded read-only planner checks remain current-statistics checks, not a substitute for populated scale-load testing.
...
## Commerce channels, delivery orchestration, and returns

Migrations `20260827_0072_channels_logistics_returns.sql`, `20260827_0073_channels_logistics_fk_indexes.sql`, and `20260827_0074_public_channel_visibility.sql` add the first research-driven post-0071 operational slice. Apply them in order after the composable ERP core. The slice adds merchant-owned `commerce_channels` and `channel_listings`, carrier-neutral `shipment_plans` and append-only `shipment_events`, `delivery_exceptions`, `return_logistics`, and append-only `return_logistics_events`, along with narrow audited RPCs and a bounded `get_published_channel_catalog` wrapper.

The supported channel kinds are web, social, POS, B2B, marketplace, and service. Public catalog reads remain limited by RLS to active channels and listings belonging to approved shops in active markets. No provider credential, courier API, webhook sender, external marketplace connector, or payment integration is enabled by these migrations.

Shipment transitions into dispatch, in-transit, or delivered require confirmed `payment_status = 'paid'`; payment proof is never treated as confirmation. Delivery completion updates fulfillment only when that paid state is true. Return closure records logistics state only and never creates a refund or settlement. Shipment and return event histories are append-only. Mutations require merchant ownership, valid state transitions, an audit reason, and explicit grants through the public RPC boundary.

Migration `0073` adds the six foreign-key indexes identified by the 0072 Performance Advisor scan. Migration `0074` adds the anonymous active-channel policy required by the invoker catalog wrapper while retaining approved-shop and active-market checks. After 0074, Security Advisor returned `lints: []`; the bounded RLS check confirmed RLS and at least one policy on all seven new base tables. The latest anonymous boundary run passed `142` checks and skipped `5` authenticated-role cases because isolated tokens were not supplied. Authenticated tests must use an isolated project or branch and must never create synthetic users or fixtures in this connected project.
