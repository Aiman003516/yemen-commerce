...
The final live Security Advisor result after migrations through 0071 is `lints: []`. Performance Advisor remediation through migration 0069 removed the new composable ERP foreign-key findings; the post-0071 scan contains 347 informational `unused_index` observations only, with no errors or warnings and no remaining ERP unindexed-FK findings. These observations do not change authorization semantics. Expected access-path indexes are retained for production traffic. Bounded read-only planner checks remain current-statistics checks, not a substitute for populated scale-load testing.
