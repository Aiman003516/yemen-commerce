...
import {
  ...
  parseDraftEnvelope,
  validateCatalogArgs,
  validateKnowledgeArgs,
  validateTerminologyArgs,
  validateCodArgs,
  validatePosArgs,
} from "./contract.ts";
...
const toolsFor = (appSurface: AppSurface, scopeType: string): ToolSpec[] => {
  ...
  const merchantTools: ToolSpec[] = [
    ...
    {
      name: "merchant.knowledge_search",
      description: "Search only ready, scope-allowed knowledge chunks and return provenance for the authenticated merchant shop.",
      actionClass: "read",
      parameters: {
        type: "object",
        properties: {
      ...
      validate: validateKnowledgeArgs,
      execute: async ({ supabase, scopeId }, args) => rpcRows(supabase, "ai_search_knowledge", {
        p_scope_type: "shop",
        p_scope_id: scopeId,
        p_query: args.query,
    ...
    {
      name: "merchant.terminology_search",
      description: "Search ready Arabic terminology entries for the authenticated merchant shop with bounded provenance fields.",
      actionClass: "read",
      parameters: {
        type: "object",
        properties: {
      ...
      validate: validateTerminologyArgs,
      execute: async ({ supabase, scopeId }, args) => rpcRows(supabase, "ai_search_terminology", {
        p_scope_type: "shop",
        p_scope_id: scopeId,
        p_query: args.query,
        p_limit: args.limit ?? 8,
  ...
  const developerTools: ToolSpec[] = [
    ...
    {
      name: "developer.platform_settings",
      description: "Read the active AI platform settings and gates; never changes them.",
      actionClass: "read",
      requiredCapability: "ai.settings.view",
      parameters: { type: "object", properties: {}, required: [], additionalProperties: false },
      validate: validateNoArgs,
      execute: async ({ supabase }) => rpcObject(supabase, "ai_get_platform_settings", {}),
    },
    {
      name: "developer.action_definitions",
      description: "Read the fixed AI action registry and enabled status; never changes it.",
      actionClass: "read",
      requiredCapability: "ai.action_registry.view",
      parameters: { type: "object", properties: {}, required: [], additionalProperties: false },
      validate: validateNoArgs,
      execute: async ({ supabase }) => rpcRows(supabase, "ai_list_action_definitions", {}),
    },
    {
      name: "developer.my_workflows",
      description: "Read bounded workflow status for the authenticated creator.",
      actionClass: "read",
      requiredCapability: "ai.workflow.view",
      parameters: { type: "object", properties: {}, required: [], additionalProperties: false },
      validate: validateNoArgs,
      execute: async ({ supabase }) => rpcRows(supabase, "ai_list_my_workflows", {}),
    },
    {
